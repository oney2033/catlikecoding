...
public readonly struct SmallXXHash4
{
    ...
    // 重载 SmallXXHash4 类型的加法操作符，使得可以对 SmallXXHash4 实例与整数进行加法操作
    // 将 h.accumulator 累加器字段与 v (整数) 相加，并将 v 转换为 uint 类型后执行加法
    public static SmallXXHash4 operator +(SmallXXHash4 h, int v) =>
        h.accumulator + (uint)v;

    // 提取当前 SmallXXHash4 对象的指定数量的位
    // count: 要提取的位数, shift: 右移的位数
    // 首先将对象右移指定位数，然后通过位与操作提取所需数量的位
    public uint4 GetBits(int count, int shift) =>
        ((uint4)this >> shift) & (uint)((1 << count) - 1);

    // 将指定数量的位转换为 [0, 1] 范围内的浮点数
    // 首先通过 GetBits 提取指定位数的值，然后除以 (2^count - 1) 来将其归一化
